...
describe('GET /todos/:id', () => {
	...
	it('should not return todo doc owned by another user', (done) => {
		request(app)
...
describe('PATCH /todos/:id', () => {
	it('should update the todo', (done) => {
		...
		request(app)
			.patch(`/todos/${id}`)
			.set('x-auth', users[0].tokens[0].token)
			.send({text: 'Completed todo from test', completed: true})
			.expect(200)
			.expect((res) => {
	...
	it('should clear completedAt when todo is not completed', (done) => {
		...
		request(app)
			...
			.send({text: 'Uncompleted todo from test', completed: false})
			.expect(200)
			.expect((res) => {
				expect(res.body.todo.text).toNotBe(todos[0].text);
...
describe('POST /users', () => {
	...
	it('should return error if request is invalid', (done) => {
		var email = 'asd';
		var password = '123';
...
describe('DELETE /users/me/token', () => {
	it('should remove the specified token', (done) => {
		request(app)
